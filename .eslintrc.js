module.exports = {
  ...
  // eslint要继承的规则，如果需要，也可以继承eslint的默认规则eslint:recommended，可查看 https://cn.eslint.org/docs/rules/
  // 项目中引入了eslint-config-prettier(https://github.com/prettier/eslint-config-prettie)，会关闭与Prettier冲突的配置
  // 并且引入了eslint-plugin-prettier(https://github.com/prettier/eslint-plugin-prettier)，可以使用eslint的规则来运行Prettier，如果想要的格式与Prettier的输出不相符，可以使用prettier-eslint(https://github.com/prettier/prettier-eslint)等其他插件提花此插件
  extends: [
    "standard",
  ...
  parserOptions: {
    ecmaVersion: 12, // ES的版本，默认为5
    parser: "@typescript-eslint/parser", // ESLint使用的解析器, 默认情况下ESLint使用Espree解析器
    sourceType: "module", // 指定源代码类型，script | module，默认为script。
  },
  plugins: ["vue", "@typescript-eslint", "html"],
  settings: {
    "html/html-extensions": [".html", ".we"], // consider .html and .we files as HTML
  },
  // 校验规则，规则说明可见 http://www.verydoc.net/eslint/00003312.html
  // off | 0 :表示关闭规则。warn | 1 :表示将该规则转换为警告。error | 2 :表示将该规则转换为错误。
  rules: {
    ...
    // 如果设置为always 那么就应该在写数组是前后都留空格
    "array-bracket-spacing": [2, "never"],
    // 是否在对象的大括号前后添加空格，包括解构赋值和导入/导出说明符。设置成always，即大括号中加入空格
    ...
    // 这个就是关于用什么来缩进了，此处指定为缩进2个空格=1个tab."SwitchCase"（默认：0）强制缩进级别case的条款switch声明
    // 详见 http://www.verydoc.net/eslint/00003359.html
    indent: [
      2,
      2,
      {
    ...
    // 此规则强制在 JSX 属性中一致使用单引号。
    "jsx-quotes": [2, "prefer-single"],
    // 此规则强制在对象字面量属性中的键和值之间保持一致的间距。在长行的情况下，允许在允许空格的地方添加新行。
    // 此处规定了在对象字面量语法中，key和value之间的空白，冒号前不要空格，冒号后面需要一个空格
    "key-spacing": [
      2,
      {
    ...
    // 禁止使用没有定义的变量，除非在／＊global＊／已经申明， 0为关闭此校验
    "no-undef": 0,
    // 禁止把undefined赋值给一个变量
    "no-undef-init": 2,
    // 禁止在不需要分行的时候使用了分行
    ...
    // 函数圆括号前面不需要加空格，配置详情可见 https://cloud.tencent.com/developer/section/1135831
